...
- Allow the direct Fronius controller to coordinate multiple Marstek batteries in one aggregate feedback loop, communicate with them concurrently on shared or separate API ports for faster reactions, avoid charging one while another discharges, hand unused power from a full or empty battery directly to a responsive one, keep reachable batteries operating when another stops responding, and return a battery that briefly falls back to Manual mode to its balanced share.

- **Fixed** direct Windows operation and Fronius load-position meter feedback by including the Marstek's locally reported AC power, allowing phase diagnosis to see the battery's test pulse, automatically recovering from a stalled local battery connection, accepting slow Marstek replies that arrive after a retry was sent instead of discarding them, retrying lost UDP calls without an extra pause on top of the reply timeout (configurable via `--api-request-attempts` and `--api-timeout`) while keeping the Passive command duration long enough to outlive a fully retried cycle, and documenting resilient Windows startup that survives power-source changes and restarts failures.
